...
const TeamMember = ({ name, src, link }) => {
    return (
        <div className="flex flex-col bg-white rounded-xl items-center py-4 my-4 shadow-white shadow-md ">
            <img src={src} className={`rounded-full w-[350px] h-[350px] object-cover ${name === 'Mirel' ? 'object-center' : (name === 'Alexandru' || name === 'George' || name === 'Vasi') ? 'object-top' : 'object-bottom'}`} alt={`Barber ${name} portrait`} />
